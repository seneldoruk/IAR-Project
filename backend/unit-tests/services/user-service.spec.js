...
const {
  ...
  resetMockedMongoDB,
  closeMockedMongoDB,
} = require("../support/mongodb-mocking");
...
describe("auth-service unit-tests", function () {
  ...
  describe("user creation tests", function () {
    it("insert user to db", async function () {
      ...
      await expect(db.collection("users").findOne())
        .to.eventually.excluding(["_id", "password"])
        .be.eqls(demouser);
    });
    ...
    it("expect correct objectId to be returned", async function () {
      ...
      ).to.eventually.have.property("_id", oid);
    });
  ...
  describe("user lookup tests", function () {
    it("expect correct user to found", async function () {
      ...
      await expect(userService.get(db, demouser.username))
        .to.eventually.excluding("_id")
        .be.eqls(demouser);
    });
  ...
  describe("credential verification test", function () {
    it("expect user to be returned if credentials match", async function () {
      ...
      await expect(
        userService.verify(
          db,
          new Credentials(demouser.username, demouser.password)
        )
      )
        .to.eventually.excluding(["_id", "password"])
        .be.eqls(demouser);
    ...
    it("expect error to be thrown if password does not match", async function () {
      ...
      await expect(
        userService.verify(
          db,
          new Credentials(demouser.username, demouser.password + "abc")
        )
      ).to.eventually.be.rejectedWith(Error);
    });
  });
